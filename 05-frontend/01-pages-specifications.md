...
*   **لوحة تحكم المستخدم (User Dashboard):** تم البدء في تطويرها.
*   **صفحات النائب والمرشح والمواطن:** تم تطوير المكونات الأساسية والصفحات التجريبية وربطها بـ Supabase.
*   **نظام الشكاوى:** تم تفعيله وربطه بـ Supabase.
*   **نظام الرسائل:** تم تفعيله وربطه بـ Supabase.
*   **عداد الزوار:** تم تفعيله وربطه بـ Supabase.
...
*   `src/components/`: يضم المكونات القابلة لإعادة الاستخدام (مثل Header, Footer, NewsTicker, AuthTabs, ComplaintModal, VisitorCounter).
*   `src/utils/`: يحتوي على دوال مساعدة (مثل `auth.js`, `citizenApi.js`, `governorates.js`, `complaintApi.js`, `visitorApi.js`).
...
## ⚙️ مواصفات الأنظمة الجديدة

### **1. نظام الشكاوى (Complaint System)**
*   **الهدف:** تمكين المواطنين من إرسال شكاوى مفصلة مع مرفقات إلى الإدارة.
*   **الميزات:**
    *   **نموذج شكوى منبثق (`ComplaintModal`):** يظهر عند النقر على زر "تسجيل شكوى" في الهيدر.
    *   **محتوى الشكوى:** نص الشكوى بحد أقصى 1500 حرف.
    *   **المرفقات:** إمكانية رفع ما يصل إلى 10 مرفقات (صور، مستندات) لكل شكوى.
    *   **مسار الشكوى:**
        1.  **إرسال المواطن:** يرسل المواطن الشكوى مباشرة إلى الأدمن.
        2.  **إدارة الأدمن:** يستقبل الأدمن الشكوى في لوحة التحكم الخاصة به.
        3.  **إعادة التوجيه:** يقوم الأدمن بإعادة توجيه الشكوى إلى نائب أو مرشح يختاره.
        4.  **منصة النائب/المرشح:** تظهر الشكوى في منصة إدارة الشكاوى الخاصة بالنائب/المرشح.
        5.  **خيارات النائب/المرشح:** يمكن للنائب/المرشح:
            *   **قبول الشكوى:** وتحديد موعد لحلها.
            *   **رفض الشكوى.**
            *   **تعليق الشكوى للدراسة:** لمدة يحددها (مثلاً 3 أيام أو أسبوع).
        6.  **زيادة النقاط:** إذا قام النائب/المرشح بحل الشكوى ووافق الأدمن على الحل، تزداد نقاط النائب/المرشح بنقطة واحدة.
*   **الربط بـ Supabase:** يتم تخزين الشكاوى ومرفقاتها في جداول `complaints` و `complaint_attachments`، مع سياسات RLS لضمان الأمان.

### **2. نظام الرسائل (Messaging System)**
*   **الهدف:** تمكين المواطنين من إرسال رسائل مباشرة إلى المرشحين/النواب.
*   **الميزات:**
    *   **زر إرسال رسالة:** متاح في صفحات المرشحين والنواب.
    *   **محتوى الرسالة:** نص الرسالة بحد أقصى 500 حرف.
    *   **لا يوجد مرفقات:** لا يمكن إرفاق مستندات مع الرسائل.
    *   **مسار الرسالة:**
        1.  **إرسال المواطن:** يرسل المواطن الرسالة مباشرة إلى المرشح/النائب.
        2.  **منصة النائب/المرشح:** تظهر الرسالة في منصة إدارة الرسائل الخاصة بالنائب/المرشح.
*   **الربط بـ Supabase:** يتم تخزين الرسائل في جدول `messages`، مع سياسات RLS لضمان الأمان.

### **3. عداد الزوار (Visitor Counter)**
*   **الهدف:** تتبع وعرض عدد الزوار للصفحات.
*   **الميزات:**
    *   **مكون `VisitorCounter`:** يعرض عدد الزوار الحالي للصفحة.
    *   **تسجيل الزيارات:** يتم تسجيل كل زيارة في قاعدة البيانات.
    *   **عرض في الهيدر والصفحة الرئيسية:** يظهر العداد في الهيدر وفي الصفحة الرئيسية.
*   **الربط بـ Supabase:** يتم تخزين بيانات الزيارات في جداول `page_views` و `visitor_counts`، مع سياسات RLS لضمان الأمان.

---

## 🚧 المهام المتبقية (وفقًا لـ `TODO.md`)

تركز المهام المتبقية بشكل أساسي على تطوير وتحسين لوحة تحكم المستخدم، بالإضافة إلى بعض التحسينات العامة:
